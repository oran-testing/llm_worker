...
class Validator:
    ...
    def validate(self, raw_str):
        raise RuntimeError("validate(self, raw_str) must be implemented by derived class")
        ...
        try:
            ...
            with open(debug_file, "a") as f:
                json.dump({
                    ...
                    "raw_response": self.raw_response if hasattr(self, 'raw_response') else "",
                    "errors": self.errors,
                    "metrics": self.metrics,
                    "parsed_data": self.parsed_data if hasattr(self, 'parsed_data') else None
                }, f)
                f.write("\n")
    ...
    def _finalize_metrics(self):
        ...
        known_fields_by_type = {
            ...
            "rtue": [
                "id", "rf_freq_offset", "rf_tx_gain", "rf_rx_gain",
                "rf_srate", "rf_nof_antennas", "rf_device_name",
                "rf_device_args", "rf_time_adv_nsamples", "rat_eutra_dl_earfcn",
                "rat_eutra_nof_carriers", "rat_nr_bands", "rat_nr_nof_carriers",
                "rat_nr_max_nof_prb", "rat_nr_nof_prb", "pcap_enable",
                "pcap_mac_filename", "pcap_mac_nr_filename", "pcap_nas_filename",
                "log_all_level", "log_phy_lib_level", "log_all_hex_limit",
                "log_filename", "log_file_max_size", "usim_mode", "usim_algo",
                "usim_opc", "usim_k", "usim_imsi", "usim_imei", "rrc_release",
                "rrc_ue_category", "nas_apn", "nas_apn_protocol", "gui_enable",
                "gw_ip_devname", "gw_ip_netmask", "general_metrics_influxdb_enable",
                "general_metrics_influxdb_url", "general_metrics_influxdb_port",
                "general_metrics_influxdb_org", "general_metrics_influxdb_token",
                "general_metrics_influxdb_bucket", "general_metrics_period_secs",
                "general_ue_data_identifier"
            ]
        }
    ...
    def _extract_json(self, raw_str):
        ...
        for open_bracket, close_bracket in bracket_pairs:
            start = raw_str.find(open_bracket)
            end = raw_str.rfind(close_bracket)
            if start != -1 and end != -1:
                candidate = raw_str[start:end + 1].strip()
                try:
...
class ResponseValidator:
    ...
    def _extract_json(self, raw_str: str) -> Optional[dict]:
        fenced_blocks = re.findall(r"```(?:json)?\n([\s\S]*?)```", raw_str)
        for block in fenced_blocks:
            block = block.strip().replace("'", "\"")
            try:
    ...
    def validate(self) -> Optional[dict]:
        self.parsed_data = self._extract_json(self.raw_response)
        if not self.parsed_data:
            self._finalize_metrics()
            return None
        self.last_parsed_json = self.parsed_data
        ...
        rtue_full_schema = {
            ...
            "gw_ip_devname": str, "gw_ip_netmask": str,
            "general_metrics_influxdb_enable": bool, "general_metrics_influxdb_url": str,
            "general_metrics_influxdb_port": int, "general_metrics_influxdb_org": str,
            "general_metrics_influxdb_token": str, "general_metrics_influxdb_bucket": str,
            "general_metrics_period_secs": float, "general_ue_data_identifier": str
        }
        ...
        if self.config_type == "jammer":
            self._validate_schema(self.parsed_data, jammer_schema, list(jammer_schema.keys()))
            self._validate_jammer_values(self.parsed_data)

        elif self.config_type == "sniffer":
            self._validate_schema(self.parsed_data, sniffer_schema, list(sniffer_schema.keys()))
            self._validate_sniffer_values(self.parsed_data)

        elif self.config_type == "rtue":
            self._validate_schema(self.parsed_data, rtue_full_schema, rtue_required_keys)
